...
export class LoadBalancerManager {
     ...
     */
    static printStatus() {
        console.log("[stream_processor] === 负载均衡器状态 ===");
        console.log("[stream_processor] 配置:", LoadBalancerManager.getConfig());
        console.log("[stream_processor] 端点状态:");
        console.table(LoadBalancerManager.getStatus());
        console.log("[stream_processor] 健康的 APIs:", LoadBalancerManager.getHealthyAPIs());
        console.log("[stream_processor] 被禁用的 APIs:", LoadBalancerManager.getBlockedAPIs());
        console.log("[stream_processor] 推荐 API:", LoadBalancerManager.getRecommendedAPI());
    }
}
// 暴露简化的管理函数
