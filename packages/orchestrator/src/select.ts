...
export function selectWinner(b: Broadcast, cleanMaxGapSec: number): Selection {
  ...
  // 每个 worker 的会话数:>1 = 该 worker 在本场断流过(我们的录制器断流重连 = 新会话/新 sessionBase)。
  const sessionCount = new Map<string, number>();
  for (const m of b.members) sessionCount.set(m.workerId, (sessionCount.get(m.workerId) ?? 0) + 1);
  // 「完整录全」= 该 worker 只有一个会话(没断流)且会话内 gap ≤ 阈值(没在录制中途断)。
  const isComplete = (m: BroadcastMember): boolean =>
    (sessionCount.get(m.workerId) ?? 0) === 1 && m.rec.totalGapSec <= cleanMaxGapSec;
  ...
  // clean ⇔ 存在完整录全的 worker。false = 所有节点都断流(pipeline 据此中断+通知,不自动拼)。
