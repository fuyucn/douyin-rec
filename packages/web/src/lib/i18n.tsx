...
const DICT = {
  zh: {
    ...
    tasks: {
      empty: "暂无任务", add: "新建任务", room: "房间", openLive: "打开直播间", anchor: "主播", quality: "画质", recorder: "直播 provider", platform: "平台",
    ...
    settings: {
      ...
      douyinSection: "抖音账号", douyinPaste: "添加 / 更新抖音 Cookie",
      douyinHint: "用于礼物和入场弹幕；扫码登录不可用时也可手动粘贴。",
      kuaishouSection: "快手账号", kuaishouPaste: "添加 / 更新快手 Cookie",
      kuaishouHint: "匿名已可拿最高画质；扫码/粘贴用于降低风控，非必须。",
      biliSection: "Bilibili 录制 Cookie", biliPaste: "添加 / 更新 B站录制 Cookie",
      biliHint: "仅用于获取高画质直播流，不会复用 biliup 上传登录态。粘贴弹窗内有查找步骤。",
      biliupSection: "Bilibili 上传账号（biliup）",
      biliupHint: "独立登录态，仅用于 hub 自动投稿；由 biliup login 和 cookies.json 管理。",
      youtubeSection: "YouTube 上传账号（youtubeuploader）",
      youtubeHint: "OAuth client_secrets.json + request.token 放在 config/youtube/；详细流程见 plans/024_youtube_upload.md。",
      youtubeReady: "已就绪（client_secrets + token 均存在）",
      youtubeNeedToken: "已找到 client_secrets.json，但没有 request.token（先在本机完成一次授权）",
      youtubeNeedSecrets: "缺 client_secrets.json（先在 Google Cloud 建 OAuth client 并下载）",
    ...
    qr: {
      ...
      biliTitle: "B站扫码登录", biliDesc: "用哔哩哔哩 App 扫码，确认后保存到 Bilibili 录制 Cookie",
      ksTitle: "快手扫码登录", ksDesc: "用快手 App 扫码，确认后保存到快手 Cookie。非官方接口，可能随页面改版失效。",
      launching: "正在获取二维码…", fetchFailed: "获取二维码失败: {msg}", success: "扫码登录成功，Cookie 已保存", err: "错误: {msg}",
    ...
    paste: {
      ...
      ksTitle: "添加 / 更新快手 Cookie",
      ksDesc: "快手 Cookie 可选：匿名取流已是最高画质，粘贴仅用于降低风控。可从 https://www.kuaishou.com 登录后复制。",
      biliTitle: "添加 Bilibili Cookie",
      biliDesc: "只复制 bilibili.com 的录制账号 Cookie，用于获取 B站高画质直播流；上传账号独立。",
      biliGuideTitle: "如何只找到 B站 Cookie",
      biliOpen: "打开 B站",
      biliStep1: "在浏览器登录 B站，并保持登录状态。",
      biliStep2: "按 F12 打开开发者工具，切到 Network（网络），刷新页面并按 api.bilibili.com 筛选。",
      biliStep3: "点任意 api.bilibili.com 请求，打开 Headers（标头）→ Request Headers（请求标头）。",
      biliStep4: "找到 Cookie，只复制它后面的值；不要复制 Cookie: 这几个字。",
      biliStep5: "确认内容包含 SESSDATA= 后，粘贴到下面的输入框并保存。",
      biliFallback: "如果没有 api.bilibili.com 请求：打开 Application（应用）→ Storage → Cookies → https://www.bilibili.com，组合 SESSDATA、bili_jct、DedeUserID、DedeUserID__ckMd5 后再粘贴。",
      biliRequired: "必须包含：SESSDATA=...（建议同时保留 bili_jct、DedeUserID、DedeUserID__ckMd5）",
      biliMissingSession: "没有找到 SESSDATA，请按上方步骤复制 B站请求的完整 Cookie。",
    },
    hub: {
      common: {
      ...
      detail: {
        ...
        chipUploadYt: "上传(YouTube)", chipUploadDual: "上传(B站+YouTube)",
      ...
      jobs: {
        ...
        stepNode: { select: "选优", pull: "拉取", merge: "合并 plain", burn_danmu: "烧 danmu", burn_livechat: "烧 livechat", upload_plain: "传 B 站 plain P1", youtube_plain: "传 YouTube", append_danmu: "追 danmu P2", append_livechat: "追 livechat P3", clean_stage_src: "清理暂存源", clean_source: "清理节点源", clean_stage: "清理产物" },
      ...
      ruleDialog: {
        ...
        uploadSection: "上传 / upload", uploadToggleLabel: "自动上传 / auto upload",
        destBilibiliLabel: "B 站 / bilibili",
        destBilibiliHint: "按既有分 P 规则投稿到 B 站。",
        destYouTubeLabel: "YouTube",
        destYouTubeHint: "把合并好的 plain mp4 上传到 YouTube(需按 plans/024 完成 OAuth)。",
        ytPrivacyLabel: "YouTube 可见性 / privacy",
        ytPrivacyHint: "Google OAuth 项目未通过审核时只能上传为 private。",
        ...
        uploadDestRequired: "开启自动上传后，至少选择一个目的地（B 站 / YouTube）。",
  ...
  en: {
    ...
    tasks: {
      empty: "No tasks", add: "New task", room: "Room", openLive: "Open live room", anchor: "Streamer", quality: "Quality", recorder: "Recorder", platform: "Platform",
    ...
    settings: {
      ...
      douyinSection: "Douyin account", douyinPaste: "Add / update Douyin cookie",
      douyinHint: "Used for gift and entry danmu. Paste manually if QR login is unavailable.",
      kuaishouSection: "Kuaishou account", kuaishouPaste: "Add / update Kuaishou cookie",
      kuaishouHint: "Anonymous already gives the best quality. This login only helps avoid risk control.",
      biliSection: "Bilibili recording cookie", biliPaste: "Add / update recording cookie",
      biliHint: "Used only for high-quality recording streams. It never reuses the biliup upload login.",
      biliupSection: "Bilibili upload account (biliup)",
      biliupHint: "Separate login used only for Hub uploads; managed by biliup login and cookies.json.",
      youtubeSection: "YouTube upload account (youtubeuploader)",
      youtubeHint: "OAuth client_secrets.json + request.token live under config/youtube/; see plans/024_youtube_upload.md.",
      youtubeReady: "Ready (client_secrets + token both present)",
      youtubeNeedToken: "client_secrets.json found, but request.token is missing (finish OAuth once locally)",
      youtubeNeedSecrets: "client_secrets.json missing (create an OAuth client in Google Cloud first)",
    ...
    qr: {
      ...
      biliTitle: "Bilibili QR Login", biliDesc: "Scan with the Bilibili app; the cookie is saved to the Bilibili recording cookie.",
      ksTitle: "Kuaishou QR Login", ksDesc: "Scan with the Kuaishou app. Non-official endpoints; may break on upstream page changes.",
      launching: "Fetching QR code…", fetchFailed: "Failed to get QR: {msg}", success: "QR login OK, cookie saved", err: "Error: {msg}",
    ...
    paste: {
      ...
      ksTitle: "Add / update Kuaishou cookie",
      ksDesc: "Kuaishou cookie is optional: anonymous recording already uses the best available quality. Paste helps avoid risk control.",
      biliTitle: "Add Bilibili cookie",
      biliDesc: "Copy a Bilibili recording-account cookie for higher-quality streams. Upload authentication is separate.",
      biliGuideTitle: "How to find only the Bilibili cookie",
      biliOpen: "Open Bilibili",
      biliStep1: "Sign in to Bilibili in your browser and stay logged in.",
      biliStep2: "Press F12, open Network, refresh the page, and filter by api.bilibili.com.",
      biliStep3: "Open any api.bilibili.com request, then Headers → Request Headers.",
      biliStep4: "Find Cookie and copy only the value after it. Do not copy the Cookie: label.",
      biliStep5: "Make sure the value contains SESSDATA=, then paste it below and save.",
      biliFallback: "If no api.bilibili.com request appears: open Application → Storage → Cookies → https://www.bilibili.com, combine SESSDATA, bili_jct, DedeUserID, and DedeUserID__ckMd5, then paste the result.",
      biliRequired: "Required: SESSDATA=... (also recommended: bili_jct, DedeUserID, DedeUserID__ckMd5)",
      biliMissingSession: "SESSDATA was not found. Follow the steps above and copy the full Bilibili request cookie.",
    },
    hub: {
      common: {
      ...
      detail: {
        ...
        chipUploadYt: "Upload (YouTube)", chipUploadDual: "Upload (Bilibili + YouTube)",
      ...
      jobs: {
        ...
        stepNode: { select: "Select", pull: "Pull", merge: "Merge plain", burn_danmu: "Burn danmu", burn_livechat: "Burn livechat", upload_plain: "Upload Bilibili plain P1", youtube_plain: "Upload YouTube", append_danmu: "Append danmu P2", append_livechat: "Append livechat P3", clean_stage_src: "Clean staged src", clean_source: "Clean node src", clean_stage: "Clean products" },
      ...
      ruleDialog: {
        ...
        uploadSection: "Upload", uploadToggleLabel: "Auto upload",
        destBilibiliLabel: "Bilibili",
        destBilibiliHint: "Uploads to Bilibili with the existing multi-part rules.",
        destYouTubeLabel: "YouTube",
        destYouTubeHint: "Uploads the merged plain mp4 to YouTube (finish OAuth per plans/024 first).",
        ytPrivacyLabel: "YouTube privacy",
        ytPrivacyHint: "Unverified Google OAuth projects can only keep videos private.",
        ...
        uploadDestRequired: "Auto upload needs at least one destination (Bilibili or YouTube).",
