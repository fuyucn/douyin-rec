...
| `--out <dir>` | `<DOUYIN_REC_ROOT>/recordings`(未设根默认 `./output-data/recordings`) | 输出目录 |
...
| `--desc <s>` | - | 稿件简介 |
| `--cookies-file <path>` | `<DOUYIN_REC_ROOT>/config/biliup/cookies.json`(未设根默认 `./output-data/...`) | biliup 登录后的 cookies |
