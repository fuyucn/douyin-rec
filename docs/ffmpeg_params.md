...
| 参数 | 值 | 作用 | 影响 |
|------|----|------|------|
| `-c copy` | — | 不转码，直接复制音视频流 | CPU 占用极低（<5%）；保留原始编码质量；不支持帧精确剪切 |
...
| 参数 | 值 | 作用 | 影响 |
|------|----|------|------|
| `-flush_packets` | `1` | 每个包写完后立即 flush 到磁盘 | 崩溃时减少数据丢失；代价是 I/O 次数增加（直播场景可接受） |
...
| `-mpegts_flags +resend_headers` | — | 每段开头重发 PAT/PMT 表 | 使每段 TS 文件可独立播放，不依赖前一段 |
