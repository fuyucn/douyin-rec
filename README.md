...
> 详细:**[docs/cli.md](./docs/cli.md)**(完整 CLI) · **[docs/app.md](./docs/app.md)**(app / web 层)。
...
- **Docker** 固定 `DOUYIN_REC_ROOT=/output-data`（容器内路径，与代码默认根同名；compose 挂载卷映射到宿主机 `docker-data/`，见下）——不受上述默认值影响。
- 专用 env（`DOUYIN_REC_DB` / `DOUYIN_REC_OUTPUT` / `BILIUP_COOKIE`）可单独覆盖某一项，优先级最高。
- **裸跑支持 `.env`**：`node dist/douyin-rec.mjs ...` 启动会自动读 `<cwd>/.env`（Node 24 内置
  `process.loadEnvFile`，无 `.env` 静默跳过）。和 docker 用的是同一份格式，本地想固定
  `DOUYIN_REC_ROOT` 不想每次手动 `export`，抄一份 `.env.example` 改改即可。docker 不受影响
  （容器内没有这个文件，且它的根固定写死在 `docker-compose.yml`）。

## 时区（`settings.timezone`）

时区由 **config 决定，不看 host/容器的 `TZ` 环境变量**——`task serve` 启动时读数据库里的
`settings.timezone`（未设 → 默认 `America/Los_Angeles`），**覆盖** `process.env.TZ`，启动日志打一行
`[tz] 时区 = ...` 可查。改用 `GET`/`POST /api/timezone`，立即生效（daemon 下一次 tick 就用新值，不用重启）；
设非法 IANA 时区名会被拒绝（400），不会让打错的值悄悄让所有 schedule 窗口算错。

- **为什么不用 env 控制**：host/容器层的 `TZ` 很难从进程外部内省——`docker compose exec ... date` /
  `ssh <host> date` 看到的是那条临时 shell 自己的环境，不是目标服务进程实际用的（得挖
  `/proc/<pid>/environ` 才能确认），曾因此误判过一次生产节点的时区。config 驱动后唯一真理源就是
  这一个设置，启动日志/API 一眼可查，不用再猜。
- docker-compose.yml / systemd 里仍保留的 `TZ=` 只是**容器/服务启动前几毫秒的初始值**（外加给
  `date` 等诊断命令一个参考），我们自己的代码启动后会立刻覆盖它——不用去改那些文件。
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `TZ` | `America/Los_Angeles` | 仅作容器启动前几毫秒 + `date` 等诊断命令的参考值；**调度实际用的时区由 config 决定**(`settings.timezone`,启动时 `applyTimezone()` 覆盖 `process.env.TZ`,不看这个 env)。查/改走 `GET`/`POST /api/timezone`,或看启动日志 `[tz] 时区 = ...` |
